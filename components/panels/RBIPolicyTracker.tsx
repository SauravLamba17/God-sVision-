  ...
  return (
    <div style={{ fontFamily: 'IBM Plex Mono', border: '1px solid #1e293b', borderLeft: '2px solid #FF9933', background: 'var(--bg-panel)' }}>
      ...
      <div style={{ padding: '8px 10px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        ...
        <div>
          ...
          {[
            ...
            <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 9, fontWeight: 700, color: '#FF9933' }}>{item.value.toFixed(2)}{item.unit}</span>
            </div>
          ))}
        </div>
        ...
        {/* Stance + Next Meeting */}
        <div>
          <div style={{ fontSize: 8, color: 'var(--text-muted)', marginBottom: 6, letterSpacing: '0.08em' }}>STANCE</div>
          <div style={{ fontSize: 12, fontWeight: 700, color: stanceColor, marginBottom: 8 }}>{data.rbiStance}</div>
          <div style={{ fontSize: 8, color: 'var(--text-muted)', marginBottom: 4, letterSpacing: '0.08em' }}>NEXT MPC</div>
          <div style={{ fontSize: 9, color: 'var(--text-primary)' }}>{new Date(data.nextMPC.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</div>
          <div style={{ fontSize: 8, color: 'var(--text-muted)', marginTop: 2 }}>Resolution: {new Date(data.nextMPC.resolution).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</div>
          <div style={{ fontSize: 8, color: '#FF9933', marginTop: 4 }}>{data.nextMPC.daysAway}d away</div>
        </div>
      </div>

      {/* Rate History mini bar chart */}
      <div style={{ padding: '0 10px 10px', borderTop: '1px solid #1b2e1b' }}>
        <div style={{ fontSize: 8, color: 'var(--text-muted)', margin: '6px 0 4px', letterSpacing: '0.08em' }}>RATE HISTORY (last 6)</div>
        ...
        <div style={{ fontSize: 7, color: 'var(--text-muted)', marginTop: 2 }}>{data.lastPolicyAction}</div>
      </div>
    </div>
  )
}
